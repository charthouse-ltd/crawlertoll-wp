...
$crawlertoll_is_enabled = ! empty( $settings['enabled'] );
$crawlertoll_price_dollars = number_format( (int) $settings['price_micros'] / 1000000, 4 );
$crawlertoll_site_url = home_url();
?>
...
<form method="post" action="options.php">
	...
	<div class="ct-status-bar">
		<div class="ct-stat-card">
			<div class="ct-stat-icon <?php echo $crawlertoll_is_enabled ? 'green' : 'amber'; ?>">
				<span class="dashicons <?php echo $crawlertoll_is_enabled ? 'dashicons-shield' : 'dashicons-shield-alt'; ?>"></span>
			</div>
			<div class="ct-stat-value"><?php echo $crawlertoll_is_enabled ? esc_html__( 'Active', 'crawlertoll' ) : esc_html__( 'Paused', 'crawlertoll' ); ?></div>
		...
		<div class="ct-stat-card">
			...
			<div class="ct-stat-value">$<?php echo esc_html( $crawlertoll_price_dollars ); ?></div>
		...
		<div class="ct-toggle-row">
			<label class="ct-toggle">
				<input
					...
					value="1"
					<?php checked( $crawlertoll_is_enabled ); ?>
				/>
				<span class="ct-toggle-slider"></span>
			</label>
			<span class="ct-toggle-label">
				<?php echo $crawlertoll_is_enabled ? esc_html__( 'CrawlerToll is active — AI crawlers are being detected and enforced.', 'crawlertoll' ) : esc_html__( 'CrawlerToll is paused — all traffic passes through normally.', 'crawlertoll' ); ?>
			</span>
		</div>
	</div>
				...
				<td>
					...
					<p class="description">
						...
						printf(
							...
							'<strong>$' . esc_html( $crawlertoll_price_dollars ) . '</strong>'
						);
						?>
					</p>
				</td>
			...
			<tr>
				...
				<td>
					<select id="crawlertoll-currency" name="<?php echo esc_attr( CRAWLERTOLL_OPTION_KEY ); ?>[currency]">
						<?php foreach ( array( 'USD', 'USDC', 'EUR', 'GBP' ) as $crawlertoll_cur ) : ?>
							<option value="<?php echo esc_attr( $crawlertoll_cur ); ?>" <?php selected( $settings['currency'], $crawlertoll_cur ); ?>>
								<?php echo esc_html( $crawlertoll_cur ); ?>
							</option>
						<?php endforeach; ?>
					</select>
				</td>
			...
			<tr>
				...
				<td>
					<select id="crawlertoll-rail" name="<?php echo esc_attr( CRAWLERTOLL_OPTION_KEY ); ?>[rail]">
						<?php foreach ( $rails as $crawlertoll_value => $crawlertoll_label ) : ?>
							<option value="<?php echo esc_attr( $crawlertoll_value ); ?>" <?php selected( $settings['rail'], $crawlertoll_value ); ?>>
								<?php echo esc_html( $crawlertoll_label ); ?>
							</option>
						<?php endforeach; ?>
					</select>
				</td>
	...
	<div class="ct-card">
		...
		<div class="ct-curl-tester">
			<div class="ct-curl-input-row">
				...
				<input type="hidden" id="ct-curl-site" value="<?php echo esc_url( $crawlertoll_site_url ); ?>" />
		...
		<div class="ct-collapse-content" style="max-height:0;">
			<div style="margin-top:16px;">
				...
				<div class="ct-bot-grid">
					<?php foreach ( $bots as $crawlertoll_bot ) : ?>
						<div class="ct-bot-chip">
							<span class="ct-bot-dot <?php echo esc_attr( $crawlertoll_bot['category'] ); ?>"></span>
							<span class="ct-bot-name"><?php echo esc_html( $crawlertoll_bot['name'] ); ?></span>
							<span class="ct-bot-op"><?php echo esc_html( $crawlertoll_bot['operator'] ); ?></span>
						</div>
					<?php endforeach; ?>
				</div>
			</div>
	...
	<div class="ct-card">
		...
		<div class="ct-endpoint-row">
			<code><?php echo esc_url( $crawlertoll_site_url . '/robots.txt' ); ?></code>
		...
		<div class="ct-endpoint-row">
			<code><?php echo esc_url( $crawlertoll_site_url . '/.well-known/context-license.json' ); ?></code>
			<span class="ct-endpoint-status active"><?php esc_html_e( 'ACTIVE', 'crawlertoll' ); ?></span>
