...
EXCLUDES=( '.git' '.gitignore' '.github' '.gstack' '.wordpress-org' 'bin' 'build' 'build.sh' 'e2e' 'tests' 'test-harness.php' 'docs' 'node_modules' 'ui/node_modules' )
...
echo "== fetch the pinned Freemius SDK (Pro build) =="
bash "$HERE/bin/fetch-freemius.sh" || { echo "  FAIL: could not fetch the Freemius SDK"; exit 1; }
...
for f in includes/class-crawlertoll-sealed.php includes/class-crawlertoll-sealed-gate.php includes/class-crawlertoll-registry.php includes/class-crawlertoll-stripe.php includes/class-crawlertoll-guard.php includes/class-crawlertoll-traffic.php includes/class-crawlertoll-cut.php includes/class-crawlertoll-premium-gate.php; do
